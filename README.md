...
### 2.5.4（versionCode 26）

- **多服务商支持**：新增 **千问 Qwen-VL / Kimi / ChatGPT**，可分别填写各自的 API Key；移除 DeepSeek Pro，仅保留 **DeepSeek V4.1 Flash**。
- **模型名更新至最新**：`deepseek-v4.1-flash`、`qwen3-vl-plus`、`kimi-k2.5`、`gpt-5`；并新增「**自定义模型名**」输入框（留空用默认）。
- **视频输入**：移除本地抽帧方案，改为**视频直传**。DeepSeek 与 ChatGPT 的 API 不支持视频，点视频按钮会提示「请切换到 千问 Qwen-VL 或 Kimi」。
- **视频抽帧设置**：由「总帧数」改为「**每秒帧数**」（10–30），长视频自动封顶并压缩画质。
- **性能修复（解决流式卡顿）**：移除固定栏 `backdrop-filter`、流式渲染改 rAF/定时节流、文本增量追加、滚动与渲染解耦、`contain:layout` 隔离重排。
- **底部导航栏**：改小并改为**白底 + 淡蓝紫渐变**。
- 版本号 **2.5.4**（versionCode 26）。
